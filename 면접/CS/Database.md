...
* Atomic
    * 트랜잭션의 작업은 모두 성공해여 commit 되거나 실패하여 rollback 되야 한다는 것을 의미합니다.
* Consistency
    * 트랜잭션이 성공적으로 완료되면 일관적인 DB 상태를 유지해야 합니다
* 독립성
    * 트랜잭션은 다른 트랜잭션에 의해 방해받아서는 안됩니다
* 내구성
    * 성공적으로 수행된 트랜잭션은 DB에 영구적으로 반영되어야 합니다
...
## 트랜잭션 격리 수준
* Read Uncommited
    * 커밋되지 않은 데이터도 조회할 수 있습니다
* read commited
    * 수행이 완료되어 커밋 되야만 조회할 수 있습니다
    * 정합성 문제가 발생할 수 있습니다
* repeatabled read
    * 트랜잭션이 시작전 커밋된 내용에 대해서만 조회 가능합니다
* serializable
    * 읽기 같은 경웨도 lock을 걸게 됩니다

## 이상 현상
릴레이션에 속성 중복으로 데이터 중복이 발생하는 것
