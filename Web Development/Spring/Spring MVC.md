...
# @RequestMapping
컨트롤러를 선택할 때 대표적으로 사용하는 어노테이션 url당 하나의 컨트롤러에 매핑되던 다른 헨들러 매핑과 달리 메서드 단위까지 세분화하여 적용할 수 있으며, url뿐 아니라 파라미터, 헤더 등 더욱 넓은 범위를 적용할 수 있다.

RequestMethod를 통해 똑같은 URL이라도 다른 메서드로 매핑해줄 수 있다.  
GET, POST, PUT, DELETE, OPTIONS, TRACE로 총 7개의 http메서드가 정의되어 있다.

    @RequestMapping(value="/", method=RequestMethod.GET)

아래와 같이 params으로도 구분할 수 있다

    // /posts?useYn=Y 일경우 호출
    @RequestMapping(value="/", params="userYn=Y")

혹시 매핑을 만족하는 메서드가 여러개인 경우 더 구현이 상세한 쪽으로 선택된다.

클래스에 공통 조건을 지정하고, 메서드 레벨에서 세분화하는 것도 가능하다.
...
사용자가 요청시 전달하는 겂을 Controller(Handler)의 매개변수로 1:1맵핑할때 사용되는 어노테이션

## @ModelAttribute
HTTP 요청에 포함된 파라미터를 모델 객체로 바인딩한다.

사용자가 요청시 전달하는 값을 객체 형태로 매핑해준다.
## @PathVariable
...
2. 클라이언트의 요청을 처리한 Controller를 작성한다
