...
merge 명령어에 병합할 커밋 이름을 넣어 실행하면, 지정한 커밋 내용이 'HEAD'가 가리키고 있는 브랜치에 넣어진다. 'HEAD'는 현재 사용중인 브랜치에 위치하게 된다. 

    git checkout master
    git merge <branchname>

'master'브랜치에 병합하기 위해서 checkout으로 master 브랜치로 전환한 다음 merge 명령어로 master 브랜치에 병합되도록 했다. 

## 브랜치 삭제하기 
병합하고 나면 만들었던 브랜치는 필요가 없기 때문에 삭제해주면 된다.

    git branch -d <branchname>

## 원격 저장소의 데이터를 로컬에 가져오기
    git pull <레포지토리주소> <브랜치명>

원격 저장소에서 데이터를 가져와 로컬 데이터에 자동으로 병합을 진행해 준다.

    git fetch <레퍼지토리주소>

 만약 데이터를 가져오기만 하고 따로 병합은 하고 싶지 않은 경우 fetch 명령어를 사용하면 된다. 해당 명령어를 사용하면 원격 저장소의 최신 이력을 이름 없는 브랜치로 로컬에 가져오게 된다. 이 브랜치는 'FETCH_HEAD'의 이름으로 체크아웃 할 수도 있다.

 만약 원격 저장소에 내용을 로컬 저장소의 master에 통합하고 싶은 경우에는 'FETCH_HEAD' 브랜치를 merge 하거나 다시 pull 을 실행하면 된다.
