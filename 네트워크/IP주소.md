...
2. 하나 이상 있다면 할당할 수 있는 지 보고 DHCP offer
    * offer 메시지
    * 자기주소(DHCP Server 주소)
    * 목적지 주소는 브로트케스트 주소
    * DHCP가 할당할 IP 주소, DHCP ID
    * DHCP 서버가 여러개면 여러 개 올 수 있다
3. 받은 offer 메시지 중 하나 선택하여 DHCP request 회신
    * 출발지는 여전히 전부 0, 출발지도 여전히 전부 1
    * 선택받지 못한 DHCP Server 들에게 알리기 위해 브로드케스트 주소로 전송

DHCP Relay Agent 필요성 : 타 네트워크의 DHCP 활용
* DHCP 서버가 타 네트워크에 있을 경우 브로드케스트 주소로도 도착하지 못함
* 라우터에 DHCP Relay Agent를 설치하여 중계함
...
* 서로 다른 네트워크에서 같은 IP 중복 사용 가능
    * 어차피 서로 연결 안함..!!!
* 인터넷 IP 주소와 엄격하게 구분
    * 사설 IP 주소는 네트워크 인터넷 연결 불가
* 임의로 할당 가능
* 사설 IP 주소 범위는 정해져 있다
...
* 내부 변환 테이블을 생성 후 필요할 때마다 연결한다
* 사설 IP 랑 공인 IP 연결, 호스트의 IP 감춘다?
...
정적 NAT
* 사설 네트워크의 특성에 따라 호스트를 미리 정한 후 공인 IP 주소와 사설 IP 주소간의 변환 테이블을 미리 정의
