...
## 읽기 전용
읽기 전용으로 설정해두면 트랜잭션 내에서 데이터를 조작하려는 시도를 막아줄 수 있을 뿐만 아니라 데이터 액세스 기술(JPA 등등)에 따라 성능이 향상될 수 있다.

JPA를 사용할 경우 Transactional이 읽기 전용이라면 영속성 컨텍스트의 변화를 체크할 필요가 없기 때문에 성능이 향상된다.
